...
const FORMAT=1,APP="0.3.19",WS=".glom/workspace.json",RES=".glom/resources.json",VIEW_DIR=".glom/views",VIEW=".glom/views/main-mindmap.json",IGNORED=new Set([".glom",".git","node_modules"]);
...
function nodesForResource(id,view=state.view){return occurrenceNodesForResource(view?.nodes||[],id)}
function nodeForResource(id,view=state.view){return primaryOccurrence(view?.nodes||[],id)}
function isAliasNode(n,view=state.view){return isSecondaryOccurrence(view?.nodes||[],n)}
...
function workspaceSettings(workspace=state.workspace){return normalizeWorkspaceSettings(workspace?.settings||{})}
function ensureWorkspaceSettings(workspace){if(!workspace)return normalizeWorkspaceSettings({});workspace.settings=normalizeWorkspaceSettings(workspace.settings||{});return workspace.settings}
function newWorkspace(name){const now=new Date().toISOString();return{format:"glom-workspace",version:FORMAT,appVersion:APP,id:uuid(),name:name||"Nouvel espace de travail",createdAt:now,updatedAt:now,defaultView:"views/main-mindmap.json",excludes:[],settings:normalizeWorkspaceSettings({})}}
...
function directResourceChildCounts(resources){
  const active=resources.filter(r=>!r.missing&&!r.excluded&&!["virtual","url"].includes(r.type)),counts=new Map(active.map(r=>[r.id,0]));
  active.forEach(r=>{const p=parentOf(r,active);if(p)counts.set(p.id,(counts.get(p.id)||0)+1)});return counts
}
function assignLayoutPositions(nodes,positions,{respectLocked=true}={}){
  const byId=new Map((nodes||[]).map(n=>[n.id,n]));for(const [id,p] of positions){const n=byId.get(id);if(!n||(respectLocked&&n.style?.locked))continue;n.x=p.x;n.y=p.y}
}
function initialLayout(nodes,edges,resources,mode){
  const rootResource=resources.find(r=>r.type==="root"),root=nodes.find(n=>n.resourceId===rootResource?.id);if(!root)return;
  assignLayoutPositions(nodes,computeTreeLayout(nodes,edges,{rootId:root.id,mode,anchor:{x:root.x,y:root.y}}),{respectLocked:false})
}
function freshView(resources,meta={},workspace=state.workspace){
  const visible=resources.filter(r=>!r.excluded),p=layout(resources),counts=directResourceChildCounts(resources),cfg=workspaceSettings(workspace),layoutMode=normalizeLayoutMode(meta.layoutMode||cfg.defaultLayoutMode,"right"),nodes=resources.map((r,i)=>({id:"n-"+r.id,resourceId:r.id,x:p.get(r.id)?p.get(r.id).x:200+(i%5)*280,y:p.get(r.id)?p.get(r.id).y:160+Math.floor(i/5)*115,collapsed:shouldAutoCollapse(counts.get(r.id)||0,cfg),occurrence:"primary",style:{}})),edges=[];
  ...
  initialLayout(nodes,edges,resources,layoutMode);
  const now=new Date().toISOString();return{format:"glom-view",version:FORMAT,id:meta.id||"main-mindmap",type:"mindmap",name:meta.name||"Carte principale",layoutMode,createdAt:now,updatedAt:now,pan:{x:40,y:40},zoom:.92,nodes:nodes,edges:edges,frames:[],objects:[]}
}
...
function mergeView(view,resources,workspace=state.workspace){
  if(!view||view.type!=="mindmap")return freshView(resources,{},workspace);
  const visible=resources.filter(r=>!r.excluded),auto=freshView(resources,{},workspace),fallback=new Map(auto.nodes.map(n=>[n.resourceId,n])),oldGroups=new Map();
  ...
  const manual=(view.edges||[]).filter(e=>e.kind==="manual"&&ids.has(e.from)&&ids.has(e.to)),frames=(view.frames||[]).map(f=>normalizeFrame(f,ids,view.edges||[])).filter(Boolean),objects=Array.isArray(view.objects)?view.objects:[];
  return Object.assign({},view,{version:FORMAT,layoutMode:normalizeLayoutMode(view.layoutMode,workspaceSettings(workspace).defaultLayoutMode),nodes,edges:hierarchy.concat(manual),frames,objects,updatedAt:new Date().toISOString()})
}
...
function renderViewSelector(){
  ...
  const ok=!!state.view;ui.viewSelect.disabled=!ok;ui.viewLayoutSelect.disabled=!ok;ui.viewLayoutSelect.value=normalizeLayoutMode(state.view?.layoutMode,workspaceSettings().defaultLayoutMode);ui.newViewBtn.disabled=!state.workspace;ui.duplicateViewBtn.disabled=!ok;ui.renameViewBtn.disabled=!ok
}
function activateView(id,{markDirty=true}={}){
  const v=allViews().find(x=>x.id===id);if(!v||v===state.view){renderViewSelector();return}
  state.view=v;state.workspace.defaultView=workspaceViewRef(v);state.selected=null;state.selectedVisual=null;state.selectedEdge=null;state.selectedFrame=null;state.linkSource=null;ui.hint.textContent="Glisser le fond pour déplacer la vue";renderViewSelector();render();transform();if(markDirty)setDirty(true)
}
function createView(){
  if(!state.workspace)return;const raw=prompt("Nom de la nouvelle carte :",uniqueViewName("Nouvelle carte"));if(raw===null)return;
  const name=uniqueViewName(raw),v=freshView(state.resources,{id:"mindmap-"+uuid(),name},state.workspace);state.views.push(v);state.view=v;state.workspace.defaultView=workspaceViewRef(v);state.selected=null;state.selectedVisual=null;state.selectedEdge=null;state.selectedFrame=null;state.linkSource=null;renderViewSelector();render();fit();setDirty(true);setStatus("Carte « "+name+" » créée","ok")
}
function duplicateView(){
  ...
  state.views.push(v);state.view=v;state.workspace.defaultView=workspaceViewRef(v);state.selected=null;state.selectedVisual=null;state.selectedEdge=null;state.selectedFrame=null;state.linkSource=null;renderViewSelector();render();transform();setDirty(true);setStatus("Carte dupliquée","ok")
}
function renameView(){
...
function prepareViews(workspace,rawViews,resources){
  const src=(rawViews||[]).filter(v=>v&&v.type==="mindmap"),views=(src.length?src:[freshView(resources,{},workspace)]).map(v=>mergeView(v,resources,workspace));const wanted=viewIdFromRef(workspace.defaultView),active=views.find(v=>v.id===wanted)||views[0];workspace.defaultView=workspaceViewRef(active);return{views,active}
}
...
function newDraftWorkspace(){
  ...
  const w=newWorkspace(name),root={id:"root-"+w.id,type:"root",path:"",title:name,tags:[],notes:"",missing:false,size:0,lastModified:null},r=[root],v=freshView(r,{},w);
...
async function addFolder(){
  ...
  const n={id:"n-"+r.id,resourceId:r.id,x:(parentNode?.x||120)+330,y:(parentNode?.y||130)+siblings*112,collapsed:false,occurrence:"primary",style:{}};
  ...
  state.treeExpanded.add(parent.id);state.treeExpanded.add(r.id);state.selected=n.id;state.selectedVisual=null;state.selectedEdge=null;state.selectedFrame=null;setDirty(true);render();setStatus("Dossier créé","ok")
}
...
async function addLocalTextFile(ext){
  ...
  const siblings=physicalChildrenOf(parent).length,n={id:"n-"+r.id,resourceId:r.id,x:(parentNode?.x||120)+330,y:(parentNode?.y||130)+siblings*112,collapsed:false,occurrence:"primary",style:{}};
  ...
  state.treeExpanded.add(parent.id);state.selected=n.id;state.selectedVisual=null;state.selectedEdge=null;state.selectedFrame=null;applyFolderSizes(state.resources);setDirty(true);render();await save(true);setStatus("Fichier "+name+" créé","ok");await openResource(r)
}
...
function syncSettingsControls(){const enabled=!!ui.autoCollapseEnabled.checked;ui.autoCollapseThreshold.disabled=!enabled;ui.settingsApplyCurrentBtn.disabled=!enabled||!state.view}
function openSettingsDialog(){
  if(!state.workspace)return;const cfg=ensureWorkspaceSettings(state.workspace);ui.autoCollapseEnabled.checked=cfg.autoCollapseLargeBranches;ui.autoCollapseThreshold.value=String(cfg.autoCollapseThreshold);ui.defaultLayoutMode.value=normalizeLayoutMode(cfg.defaultLayoutMode);syncSettingsControls();if(!ui.settingsDialog.open)ui.settingsDialog.showModal()
}
function settingsFromForm(){
  return normalizeWorkspaceSettings({...state.workspace?.settings,autoCollapseLargeBranches:ui.autoCollapseEnabled.checked,autoCollapseThreshold:ui.autoCollapseThreshold.value,defaultLayoutMode:ui.defaultLayoutMode.value})
}
function storeSettingsFromForm(){
  if(!state.workspace)return null;const before=JSON.stringify(workspaceSettings()),next=settingsFromForm();state.workspace.settings=next;if(before!==JSON.stringify(next))setDirty(true);return next
}
function visibleChildren(id){return children(id).filter(childId=>{const n=node(childId),r=n&&resource(n.resourceId);return!!(r&&!r.excluded&&!r.missing)})}
function layoutModeLabel(mode){return{right:"droite",left:"gauche",down:"bas",balanced:"gauche/droite",radial:"radiale"}[normalizeLayoutMode(mode)]}
function layoutRootId(){
  if(!state.view)return null;const rootResource=workspaceRoot(),preferred=state.view.nodes.find(n=>n.resourceId===rootResource?.id)?.id;return hierarchyRootId(state.view.nodes,state.view.edges,preferred)
}
function hierarchyParentNodeId(id){return state.view?.edges.find(e=>e.kind==="hierarchy"&&e.to===id)?.from||null}
function effectiveBranchLayout(id){
  let current=id;while(current){const n=node(current);if(n?.layoutMode)return normalizeLayoutMode(n.layoutMode);current=hierarchyParentNodeId(current)}
  return normalizeLayoutMode(state.view?.layoutMode,workspaceSettings().defaultLayoutMode)
}
function applyLayoutAt(rootId,mode,{respectLocked=true}={}){
  if(!state.view||!rootId)return;const root=node(rootId);if(!root)return;
  const positions=computeTreeLayout(state.view.nodes,state.view.edges,{rootId,mode:normalizeLayoutMode(mode),anchor:{x:root.x,y:root.y}});assignLayoutPositions(state.view.nodes,positions,{respectLocked})
}
function applyNestedLayoutOverrides(rootId){
  if(!state.view||!rootId)return;const depths=hierarchyDepths(state.view.nodes,state.view.edges,rootId),overrides=state.view.nodes.filter(n=>n.id!==rootId&&n.layoutMode&&depths.has(n.id)).sort((a,b)=>(depths.get(a.id)||0)-(depths.get(b.id)||0));
  overrides.forEach(n=>applyLayoutAt(n.id,n.layoutMode))
}
function updateViewLayoutMode(){
  if(!state.view)return;state.view.layoutMode=normalizeLayoutMode(ui.viewLayoutSelect.value,workspaceSettings().defaultLayoutMode);autoLayout()
}
function updateBranchLayoutMode(){
  const n=selectedNode();if(!n)return;const raw=ui.branchLayoutMode.value;if(raw)n.layoutMode=normalizeLayoutMode(raw);else delete n.layoutMode;setDirty(true);renderInspector();setStatus(raw?"Disposition de branche : "+layoutModeLabel(raw):"Disposition de branche héritée","ok")
}
function applySelectedBranchLayout(){
  const n=selectedNode();if(!n)return;applyLayoutAt(n.id,effectiveBranchLayout(n.id));applyNestedLayoutOverrides(n.id);setDirty(true);renderMap();fit();setStatus("Branche réorganisée — "+layoutModeLabel(effectiveBranchLayout(n.id)),"ok")
}
function applyLargeBranchCollapse(){
  if(!state.workspace||!state.view)return;const cfg=storeSettingsFromForm();let changed=0;
  if(cfg.autoCollapseLargeBranches){for(const n of state.view.nodes){if(visibleChildren(n.id).length>cfg.autoCollapseThreshold&&!n.collapsed){n.collapsed=true;changed++}}}
  if(changed)setDirty(true);renderMap();renderInspector();setStatus(changed?changed+" branche(s) volumineuse(s) repliée(s)":"Aucune nouvelle branche à replier","ok")
}
function saveSettingsDialog(ev){
  ev?.preventDefault();if(!state.workspace)return;const cfg=storeSettingsFromForm();ui.settingsDialog.close();setStatus("Paramètres enregistrés — seuil "+cfg.autoCollapseThreshold+(cfg.autoCollapseLargeBranches?" actif":" désactivé")+" · disposition "+layoutModeLabel(cfg.defaultLayoutMode),"ok")
}
...
function excludeSelectedResource(){
  ...
  if(addExclusionRule(r.path)){state.selected=null;state.selectedFrame=null;render();setStatus("Ressource exclue du workspace","ok")}
}
async function rescan(){
  if(state.mode==="fallback"){ui.folderFallback.click();return}if(state.mode!=="fs"||!state.handle)return;
  setStatus("Rescan…");const activeId=state.view?.id,s=await scan(state.handle,state.workspace);state.resources=reconcile(state.workspace,s.entries,state.resources);state.views=allViews().map(v=>mergeView(v,state.resources,state.workspace));state.view=state.views.find(v=>v.id===activeId)||state.views[0];state.scanTruncated=s.truncated;renderViewSelector();render();
...
function updateActionStates(){
  ...
  if(ui.contextFrameBtn){ui.contextFrameBtn.disabled=!hasSelection;setButtonLabel(ui.contextFrameBtn,hasFrame?"Sélectionner le groupe":"Créer un groupe")}if(ui.contextDuplicateOccurrenceBtn)ui.contextDuplicateOccurrenceBtn.disabled=!hasSelection;if(ui.contextExcludeBtn)ui.contextExcludeBtn.disabled=!hasSelection||alias||!["file","folder"].includes(r?.type)
}
function show(){
  ...
  [ui.scanBtn,ui.exclusionsBtn,ui.settingsBtn,ui.exportBtn,ui.folderBtn,ui.ideaBtn,ui.urlBtn,ui.fitBtn,ui.autoLayoutBtn].forEach(b=>b.disabled=!ok);
...
function treeRow(r,d){
  ...
  const n=nodeForResource(r.id);if(state.selected&&selectedResource()?.id===r.id)row.classList.add("selected");row.style.setProperty("--depth",d);
...
function dragVisualObject(ev,o,e){
  ...
  if(state.selectedVisual!==o.id){state.selectedVisual=o.id;state.selected=null;state.selectedEdge=null;state.selectedFrame=null;state.linkSource=null;ui.visualObjects.querySelectorAll(".visual-object.selected").forEach(x=>x.classList.remove("selected"));e.classList.add("selected");renderTree();renderInspector()}
...
function dragNode(ev,n,e){
  const source=resource(n.resourceId),style=effectiveNodeStyle(n,source),alias=isAliasNode(n);if(ev.button!==0||ev.target.closest("button")||style.locked)return;
  ev.stopPropagation();if(state.selected!==n.id||state.selectedFrame){state.selected=n.id;state.selectedFrame=null;state.selectedVisual=null;state.selectedEdge=null;ui.nodes.querySelectorAll(".node.selected").forEach(x=>x.classList.remove("selected"));ui.frames.querySelectorAll(".branch-frame.selected").forEach(x=>x.classList.remove("selected"));e.classList.add("selected");renderTree();renderInspector()}
  ...
  function mv(x){
    ...
    renderEdges();renderFrames();clearCentralDropHighlight();drop=moved&&!alias?centralDropCandidate(x.clientX,x.clientY,n.id):null;if(drop&&!groupIds.includes(nodeForResource(drop.resource.id)?.id))drop.element.classList.add("drop-target");else if(drop&&groupIds.includes(nodeForResource(drop.resource.id)?.id))drop=null
  }
...
function select(id){state.selected=id;state.selectedVisual=null;state.selectedEdge=null;state.selectedFrame=null;renderTree();renderMap();renderInspector()}
function selectVisual(id){state.selectedVisual=id;state.selected=null;state.selectedEdge=null;state.selectedFrame=null;state.linkSource=null;renderTree();renderMap();renderInspector()}
function selectEdge(id){state.selectedEdge=id;state.selected=null;state.selectedVisual=null;state.selectedFrame=null;state.linkSource=null;renderTree();renderMap();renderInspector()}
function manualEdge(a,b){
  ...
  state.selectedEdge=ed.id;state.selected=null;state.selectedVisual=null;state.selectedFrame=null;return ed
}
function renderInspector(){
  const ed=selectedEdgeObject(),vo=selectedVisualObject(),frSelected=selectedFrameObject(),r=selectedResource(),n=selectedNode();
  if(!ed)delete ui.edgeForm.dataset.edgeId;ui.noSelection.classList.toggle("hidden",!!r||!!vo||!!ed||!!frSelected);ui.form.classList.toggle("hidden",!r);ui.visualForm.classList.toggle("hidden",!vo);ui.edgeForm.classList.toggle("hidden",!ed);
  if(ed){
  ...
  ui.selectionKind.textContent=frSelected?"Groupe visuel · "+frameMemberIds(frSelected).length+" élément"+(frameMemberIds(frSelected).length>1?"s":""):r?typeLabel(r)+(n&&isAliasNode(n)?" · occurrence secondaire":""):"Aucune sélection";if(!r||!n){updateActionStates();return}
  ...
  ui.branchLayoutMode.value=n.layoutMode||"";ui.branchLayoutApplyBtn.disabled=!visibleChildren(n.id).length;
  ...
  const fr=frSelected&&frSelected.rootNodeId===n.id?frSelected:frameForNode(n.id);ui.frameFields.classList.toggle("hidden",!fr);ui.frameToggleBtn.textContent=fr?"✕ Dissoudre ce groupe":"＋ Créer un groupe depuis ce nœud";ui.branchFrameSection.classList.remove("hidden");
  if(fr){const geom=ensureFrameGeometry(fr);ui.frameTitle.value=fr.title||r.title;ui.frameFontSize.value=Number(fr.fontSize)||12;ui.frameFontSizeValue.value=(Number(fr.fontSize)||12)+" px";ui.frameFontFamily.value=fr.fontFamily||"system";ui.frameFontWeight.value=String(fr.fontWeight||"800");ui.frameItalic.checked=!!fr.italic;ui.frameTextColor.value=fr.textColor||fr.borderColor||"#6366f1";ui.frameBorderColor.value=fr.borderColor||"#6366f1";ui.frameBackgroundColor.value=fr.backgroundColor||"#eef2ff";ui.frameOpacity.value=Number(fr.opacity)||10;ui.frameOpacityValue.value=(Number(fr.opacity)||10)+" %";ui.frameBorderStyle.value=fr.borderStyle||"solid";ui.frameMemberCount.textContent=String(frameMemberIds(fr).length);ui.frameWidth.value=Math.round(geom?.w||320);ui.frameHeight.value=Math.round(geom?.h||180);ui.frameLocked.checked=!!fr.locked;ui.frameWidth.disabled=!!fr.locked;ui.frameHeight.disabled=!!fr.locked;ui.frameFitBtn.disabled=!!fr.locked;ui.frameCaptureBtn.disabled=!!fr.locked}
  ui.openResource.disabled=r.type==="virtual";ui.openResource.textContent=["root","folder"].includes(r.type)?"▦ Galerie du dossier":"👁️ Ouvrir / prévisualiser";const aliasNode=isAliasNode(n);ui.deleteBtn.classList.toggle("hidden",!aliasNode&&!["virtual","url"].includes(r.type));ui.deleteBtn.textContent=aliasNode?"🗑️ Retirer cette occurrence":"🗑️ Supprimer cette ressource";ui.collapseBtn.classList.toggle("hidden",!visibleChildren(n.id).length);ui.collapseBtn.textContent=n.collapsed?"▸ Déplier":"▾ Replier";ui.linkBtn.textContent=state.linkSource===n.id?"✕ Annuler le lien":"⛓️ Relier à…";updateActionStates()
}
...
function createBranchFrame(){
  const n=selectedNode(),r=selectedResource();if(!n||!r)return null;state.view.frames=state.view.frames||[];const existing=frameForNode(n.id);if(existing){selectFrame(existing.id);return existing}
  const fr={id:"frame-"+uuid(),rootNodeId:n.id,memberNodeIds:hierarchyDescendants(n.id),title:r.title,fontSize:12,fontFamily:"system",fontWeight:"800",italic:false,textColor:"#6366f1",borderColor:"#6366f1",backgroundColor:"#eef2ff",opacity:10,borderStyle:"solid",padding:28,locked:false};state.view.frames.push(fr);ensureFrameGeometry(fr);state.selectedFrame=fr.id;setDirty();renderMap();renderInspector();setStatus("Groupe visuel créé — ses membres sont désormais indépendants de la hiérarchie","ok");return fr
}
function selectOrCreateFrame(){const n=selectedNode();if(!n)return;const fr=frameForNode(n.id);if(fr)selectFrame(fr.id);else createBranchFrame()}
function toggleBranchFrame(){
  const n=selectedNode();if(!n)return;const fr=selectedFrameObject()||frameForNode(n.id);if(!fr){createBranchFrame();return}
  state.view.frames=(state.view.frames||[]).filter(x=>x.id!==fr.id);if(state.selectedFrame===fr.id)state.selectedFrame=null;setDirty();renderMap();renderInspector();setStatus("Groupe dissous — les nœuds sont conservés","ok")
}
function fitSelectedFrame(){
  const fr=selectedFrameObject()||frameForNode(selectedNode()?.id);if(!fr||fr.locked)return;const b=frameContentBounds(fr);if(!b)return;Object.assign(fr,b);state.selectedFrame=fr.id;setDirty();renderMap();renderInspector();setStatus("Cadre ajusté au contenu","ok")
}
function captureFrameMembers(){
  const fr=selectedFrameObject()||frameForNode(selectedNode()?.id);if(!fr||fr.locked)return;const g=ensureFrameGeometry(fr);if(!g)return;const hidden=hiddenNodes(),inside=(state.view.nodes||[]).filter(n=>!hidden.has(n.id)&&!resource(n.resourceId)?.excluded).filter(n=>{const b=nodeBox(n),cx=n.x+b.w/2,cy=n.y+b.h/2;return cx>=g.x&&cx<=g.x+g.w&&cy>=g.y&&cy<=g.y+g.h}).map(n=>n.id);
  if(!inside.includes(fr.rootNodeId))inside.unshift(fr.rootNodeId);fr.memberNodeIds=[...new Set(inside)];state.selectedFrame=fr.id;setDirty();renderMap();renderInspector();setStatus(fr.memberNodeIds.length+" nœud(s) adopté(s) par le groupe","ok")
}
function updateBranchFrame(){
  const n=selectedNode(),fr=selectedFrameObject()||(n&&frameForNode(n.id));if(!fr)return;fr.title=ui.frameTitle.value;fr.fontSize=Math.max(8,Number(ui.frameFontSize.value)||12);fr.fontFamily=ui.frameFontFamily.value;fr.fontWeight=ui.frameFontWeight.value;fr.italic=ui.frameItalic.checked;fr.textColor=ui.frameTextColor.value;fr.borderColor=ui.frameBorderColor.value;fr.backgroundColor=ui.frameBackgroundColor.value;fr.opacity=Number(ui.frameOpacity.value)||0;fr.borderStyle=ui.frameBorderStyle.value;fr.locked=ui.frameLocked.checked;fr.w=Math.max(120,Number(ui.frameWidth.value)||fr.w||320);fr.h=Math.max(80,Number(ui.frameHeight.value)||fr.h||180);ui.frameFontSizeValue.value=fr.fontSize+" px";ui.frameOpacityValue.value=fr.opacity+" %";ui.frameWidth.disabled=!!fr.locked;ui.frameHeight.disabled=!!fr.locked;ui.frameFitBtn.disabled=!!fr.locked;ui.frameCaptureBtn.disabled=!!fr.locked;setDirty();renderFrames()
}
function toggleCollapse(){const n=selectedNode();if(!n)return;n.collapsed=!n.collapsed;setDirty();renderMap();renderInspector()}
function duplicateSelectedOccurrence(){
...
function addShape(){
  ...
  state.view.objects=state.view.objects||[];state.view.objects.push(o);state.selectedVisual=o.id;state.selected=null;state.selectedEdge=null;state.selectedFrame=null;setDirty();render()
}
...
function addImageObject(r){
  ...
  state.view.objects=state.view.objects||[];state.view.objects.push(o);state.selectedVisual=o.id;state.selected=null;state.selectedEdge=null;state.selectedFrame=null;setDirty();ui.imageObjectDialog.close();render()
}
...
function addIdea(){const title=prompt("Nom de la nouvelle idée :","Nouvelle idée");if(!title||!title.trim())return;const r={id:"v-"+uuid(),type:"virtual",title:title.trim(),tags:[],notes:""},c=centerWorld(),n={id:"n-"+r.id,resourceId:r.id,x:c.x-120,y:c.y-37,collapsed:false,occurrence:"primary",style:{}};state.resources.push(r);state.view.nodes.push(n);if(state.selected)manualEdge(state.selected,n.id);state.selected=n.id;state.selectedVisual=null;state.selectedEdge=null;state.selectedFrame=null;setDirty();render()}
function addUrl(){const raw=prompt("Adresse web :","https://");if(!raw)return;let u;try{u=new URL(raw).href}catch(e){alert("URL invalide");return}const title=prompt("Titre du lien :",new URL(u).hostname)||new URL(u).hostname,r={id:"u-"+uuid(),type:"url",title:title,tags:[],notes:"",url:u},c=centerWorld(),n={id:"n-"+r.id,resourceId:r.id,x:c.x-120,y:c.y-37,collapsed:false,occurrence:"primary",style:{}};state.resources.push(r);state.view.nodes.push(n);if(state.selected)manualEdge(state.selected,n.id);state.selected=n.id;state.selectedVisual=null;state.selectedEdge=null;state.selectedFrame=null;setDirty();render()}
...
function fit(){
  ...
  state.view.zoom=z;state.view.pan.x=(r.width-spanW*z)/2-minX*z;state.view.pan.y=(r.height-spanH*z)/2-minY*z;transform()
}
function autoLayout(){
  if(!state.view)return;const rootId=layoutRootId();if(!rootId)return;state.view.layoutMode=normalizeLayoutMode(state.view.layoutMode,workspaceSettings().defaultLayoutMode);applyLayoutAt(rootId,state.view.layoutMode);applyNestedLayoutOverrides(rootId);setDirty();renderViewSelector();renderMap();fit();setStatus("Disposition réorganisée — "+layoutModeLabel(state.view.layoutMode),"ok")
}
...
function wire(){
  ...
  ui.contextFolderBtn.onclick=addFolder;ui.contextIdeaBtn.onclick=addIdea;ui.contextRelationBtn.onclick=linkMode;ui.contextFrameBtn.onclick=selectOrCreateFrame;ui.contextDuplicateOccurrenceBtn.onclick=duplicateSelectedOccurrence;ui.contextExcludeBtn.onclick=excludeSelectedResource;
  ...
  ui.copyStyleBtn.onclick=copyNodeStyle;ui.pasteStyleBtn.onclick=pasteNodeStyle;ui.resetStyleBtn.onclick=resetNodeStyle;ui.branchLayoutMode.onchange=updateBranchLayoutMode;ui.branchLayoutApplyBtn.onclick=applySelectedBranchLayout;ui.frameToggleBtn.onclick=toggleBranchFrame;ui.frameFitBtn.onclick=fitSelectedFrame;ui.frameCaptureBtn.onclick=captureFrameMembers;[ui.frameTitle,ui.frameFontSize,ui.frameFontFamily,ui.frameFontWeight,ui.frameItalic,ui.frameTextColor,ui.frameBorderColor,ui.frameBackgroundColor,ui.frameOpacity,ui.frameBorderStyle,ui.frameWidth,ui.frameHeight,ui.frameLocked].forEach(x=>{x.addEventListener("input",updateBranchFrame);x.addEventListener("change",updateBranchFrame)});
  ...
  ui.settingsClose.onclick=()=>ui.settingsDialog.close();ui.settingsForm.onsubmit=saveSettingsDialog;ui.settingsApplyCurrentBtn.onclick=applyLargeBranchCollapse;ui.autoCollapseEnabled.onchange=syncSettingsControls;ui.autoCollapseThreshold.oninput=syncSettingsControls;ui.exclusionsClose.onclick=()=>ui.exclusionsDialog.close();ui.exclusionForm.onsubmit=e=>{e.preventDefault();if(addExclusionRule(ui.exclusionPattern.value)){ui.exclusionPattern.value="";renderExclusionsList();setStatus("Règle d’exclusion ajoutée","ok")}};ui.excludeBakPreset.onclick=()=>{addExclusionRule("*.bak");renderExclusionsList()};ui.excludeNppPreset.onclick=()=>{addExclusionRule("nppBackup/**");renderExclusionsList()};
