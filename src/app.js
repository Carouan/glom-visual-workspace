...
const FORMAT=1,APP="0.3.17",WS=".glom/workspace.json",RES=".glom/resources.json",VIEW_DIR=".glom/views",VIEW=".glom/views/main-mindmap.json",IGNORED=new Set([".glom",".git","node_modules"]);
...
function mergeView(view,resources,workspace=state.workspace){
  ...
  const visible=resources.filter(r=>!r.excluded),auto=freshView(resources,{},workspace),old=new Map((view.nodes||[]).map(n=>[n.resourceId,n])),fallback=new Map(auto.nodes.map(n=>[n.resourceId,n])),nodes=resources.map(r=>{
    const fb=fallback.get(r.id),n=Object.assign({},old.get(r.id)||fb);if(typeof n.collapsed!=="boolean")n.collapsed=!!fb?.collapsed;n.style=Object.assign({},n.style||{});return n
  });
...
function openSettingsDialog(){
  if(!state.workspace)return;const cfg=ensureWorkspaceSettings(state.workspace);ui.autoCollapseEnabled.checked=cfg.autoCollapseLargeBranches;ui.autoCollapseThreshold.value=String(cfg.autoCollapseThreshold);syncSettingsControls();if(!ui.settingsDialog.open)ui.settingsDialog.showModal()
}
function settingsFromForm(){
  return normalizeWorkspaceSettings({...state.workspace?.settings,autoCollapseLargeBranches:ui.autoCollapseEnabled.checked,autoCollapseThreshold:ui.autoCollapseThreshold.value})
}
...
function saveSettingsDialog(ev){
  ev?.preventDefault();if(!state.workspace)return;const cfg=storeSettingsFromForm();ui.settingsDialog.close();setStatus("Paramètres enregistrés — seuil "+cfg.autoCollapseThreshold+(cfg.autoCollapseLargeBranches?" actif":" désactivé"),"ok")
}
...
function autoLayout(){
  if(!state.view)return;const auto=freshView(state.resources,{},state.workspace),byResource=new Map(auto.nodes.map(n=>[n.resourceId,n]));
